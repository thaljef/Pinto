...

{
  ...
  $local->registration_not_ok('AUTHOR/DistA-1/PkgA~1/init');
  $local->registration_not_ok('AUTHOR/DistB-1/PkgB~1/init');
  $local->registration_not_ok('AUTHOR/DistC-1/PkgC~1/init');
  $local->registration_not_ok('AUTHOR/DistD-1/PkgD~1/init');

  # The filesystem is not transactional, so the archive for A will still be there...
  $local->path_exists_ok( [qw(init authors id A AU AUTHOR DistA-1.tar.gz)] );

  # And so will the archives for B and D...
  $local->path_exists_ok( [qw(init authors id A AU AUTHOR DistB-1.tar.gz)] );
  $local->path_exists_ok( [qw(init authors id A AU AUTHOR DistD-1.tar.gz)] );

  # But C should not be there because we never got to pull it...
  $local->path_not_exists_ok( [qw(init authors id A AU AUTHOR DistC-1.tar.gz)] );
...

{
  ...
  $local->registration_ok('AUTHOR/DistA-1/PkgA~1/init', 'Target before failure ok');
  $local->registration_ok('AUTHOR/DistC-1/PkgC~1/init', 'Target after failure ok');

  # But B (the middle target) should not...
  $local->registration_not_ok('AUTHOR/DistB-1/PkgB~1/init', 'But failed target should not be there');

  # Nor should any of B's prereqs...
  $local->registration_not_ok('AUTHOR/DistD-1/PkgD~1/init', 'Dependency of failed target was unregisted');
  ...
  $local->path_not_exists_ok( [qw(init authors id A AU AUTHOR DistB-1.tar.gz)] );
  $local->path_not_exists_ok( [qw(init authors id A AU AUTHOR DistD-1.tar.gz)] );
}
