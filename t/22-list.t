...

{
  my $buffer = '';
  my $out = IO::String->new(\$buffer);
  $t->run_ok( List => {stack => 'qa', out => $out, pinned => 1} );
  my @lines = split /\n/, $buffer;

  is scalar @lines, 0, 'Got correct number of records in listing';
}
...

{
  my $buffer = '';
  my $out = IO::String->new(\$buffer);
  $t->run_ok( Pin => {targets => 'Baz', stack => 'qa'} );
  $t->run_ok( List => {stack => 'qa', out => $out, pinned => 1} );
  my @lines = split /\n/, $buffer;

  is scalar @lines, 1, 'Got correct number of records in listing';
  like $lines[0], qr/Baz \s+ 0.03/x, 'Listing where pinned == 1 on qa stack';
}
