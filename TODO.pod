...
=item Get tests passing again

=item Fix locking mechanism (lock per stack, shared/exclusive, maybe in db).

=item Eliminate denormalized data (unecessary preoptimization).

=item Rename RegistrationHistory to RegistrationChange.

=item Make commit messages mandatory.  Generate message stub, shell out to editor

=item Consider moving location of shared authors dir.

=item Write tests for versioning code.
...
=item Option to initialize bare repository (i.e. no initial stack)
...
=item Option to specify name/description of the initial stack
