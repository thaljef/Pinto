...
=item Decide how to check for changes in stack (checksum? object attribute? table column?)
...
=item Implement a diff class.

=item Decide how to do commit digests.

=item Make locks into a column in the stack table

=item Add asserts to prevent modifying a finalized commit

=item Add asserts to ensure a commit is finalized

=item Put username back into the log file
...
=item VACUUM and ANALYZE db during "clean"
