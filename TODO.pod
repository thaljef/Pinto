...
=item Eliminate VCS backend

=item Eliminate the Batch -- just one action at a time

=item Allow Actions to take multiple targets

=item Log all exceptions (no confessing)

=item Add hooks before/after adding and pulling

=item Ensure there is an open revision before each Repos operation

=item Limit each revision to a stack
...
=item Keep track of dependencies
