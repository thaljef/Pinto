...
=item Test cases for Reindex, Deindex, Delete

=item Rename columns in the kommit table

=item Report timestamps in the local timezone

=item Move more behavior into schema classes?
...
=item Way to pin/unpin all packages in a stack (or maybe a command to "lock" at stack)
