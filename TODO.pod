...
=item Command to list dependencies and dependants

=item Way to verify prereqs on a stack

=item Command to unregister a pkg/dist ??

=item Option to enforce strict stacks (no overlapping dists)

=item Re-examine interface classes

=item Write test cases for Stack operations

=item Create a type for StackPropertyName

=item Prohibit deleting default stack

#--------------------------

=item Re-examine log levels for messages

=item Divide actions into Actions and Reports

=item Create Request (or Event) superclass for Action and Report

=item Verify checksums during 'verify'

=item Enable shared (for Reports) and exclusive locking (for Actions)
...
=item Way to view all packages (not just in a stack)
...
=item Extract new Roles into Pinto-Common
